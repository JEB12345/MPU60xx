...
uint8_t IMU_Init(uint32_t i2cFreq, uint32_t sysFreq)
{
    ...
    i2c_internal_ret_count = RET_ERROR;
    return 0;
}
...
void IMU_CopyI2CData(MPU6050_Data *mpuData, MAG3110_Data *magData)
{
    ...
    if (i2c_internal_ret == RET_OK && flag) {
        flag = 0;
...
void IMU_GetData()
{
    ...
    if (i2c_internal_ret == RET_OK || i2c_internal_ret == RET_ERROR) {
        //start new read
        if (i2c_internal_ret_count == RET_OK) {
            cnt = (i2c_internal_count[0] << 8) | i2c_internal_count[1];
//            if (cnt >= 14) {
...
void IMU_GetCount()
{
//    if (i2c_internal_ret_count == RET_OK || i2c_internal_ret_count == RET_ERROR) {
//        //start new read
//        i2c_internal_ret_count = RET_UNKNOWN;
//        i2c_1_read(MPU60XX_ADDRESS, RA_FIFO_COUNT_H, 2, i2c_internal_count, &i2c_internal_ret_count);
//        //        i2c_1_read(MPU60XX_ADDRESS, RA_FIFO_R_W, 14, i2c_internal_data, &i2c_internal_ret);
//    }
